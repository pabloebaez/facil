...
import { daviplataPaymentService } from '../../services/api';

export function DaviplataQRModal({ isOpen, onClose, amount, onPaymentConfirmed }) {
  ...
  const generateQR = async () => {
    try {
      ...
      const response = await daviplataPaymentService.generateQR(amount, reference);
      ...
      setError(err.response?.data?.error || err.response?.data?.message || 'Error al generar código QR de Daviplata');
    } finally {
      setLoading(false);
    }
  };
  ...
  const startStatusCheck = (reference) => {
    const interval = setInterval(async () => {
      ...
      try {
        ...
        const response = await daviplataPaymentService.checkStatus(reference);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800">Pago con Daviplata</h2>
          <button
            onClick={onClose}
          ...
          <div className="text-center">
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Escanea el código QR con la app de Daviplata
              </p>
            </div>

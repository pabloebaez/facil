...
import { nequiPaymentService } from '../../services/api';

export function NequiQRModal({ isOpen, onClose, amount, onPaymentConfirmed }) {
  ...
  const generateQR = async () => {
    try {
      ...
      const response = await nequiPaymentService.generateQR(amount, reference);
      ...
      setError(err.response?.data?.error || err.response?.data?.message || 'Error al generar código QR de Nequi');
    } finally {
      setLoading(false);
    }
  };
  ...
  const startStatusCheck = (reference) => {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        const response = await nequiPaymentService.checkStatus(reference);
        if (response.data.success && response.data.status === 'completed') {
          setPaymentStatus('completed');
          clearInterval(interval);
          // Esperar 1 segundo antes de cerrar y confirmar
          setTimeout(() => {
    ...
    // Limpiar intervalo cuando el componente se desmonte o se cierre el modal
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800">Pago con Nequi</h2>
          <button
            onClick={onClose}
